...
| __分類__    |                 | __描述__ |
|-----------------|-----------------|-----------------|
| 值類型     | 簡單型別    | 帶正負號整數類資料： `sbyte`， `short`， `int`， `long` |
|                 |                 | 不帶正負號的整數類資料： `byte`， `ushort`， `uint`， `ulong` |
|                 |                 | Unicode 字元： `char` |
|                 |                 | IEEE 浮點數： `float`， `double` |
|                 |                 | 高精確度十進位︰ `decimal` |
|                 |                 | 布林值： `bool` |
|                 | 列舉型別      | 使用者定義型別，格式 `enum E {...}` |
|                 | 結構型別    | 使用者定義型別，格式 `struct S {...}` |
|                 | 可為 Null 的型別  | 含有 `null` 值的所有其他數值型別的擴充 |
| 參考型別 | 類別型別     | 所有其他類型的 ultimate 基底類別： `object` |
|                 |                 | Unicode 字串： `string` |
|                 |                 | 使用者定義型別，格式 `class C {...}` |
|                 | 介面型別 | 使用者定義型別，格式 `interface I {...}` |
|                 | 陣列型別     | 單一和多維度，例如`int[]`和 `int[,]` |
...
| __分類__      | __Bits__ | __類型__  | __範圍/精確度__ |
|-------------------|----------|-----------|---------------------|
...
| 指派或匿名函式 | `x = y`           | 指派 |
|                                  | `x op= y`         | 複合指派；支援的運算子包括 `*=` `/=` `%=` `+=` `-=` `<<=` `>>=` `&=` `^=` <code>&#124;=</code> |
