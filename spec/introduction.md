...
數個 C# 功能有助於建構健全且持久的應用程式：***回收***自動回收未使用的物件; 所佔用的記憶體***例外狀況處理***提供錯誤偵測和復原; 的結構化和可延伸方法並***型別安全***語言設計，因此無法讀取未初始化要超過其範圍中，或執行未檢查的類型轉換的索引陣列的變數。
...
| __分類__    |                 | __描述__ |
|-----------------|-----------------|-----------------|
| 值類型     | 簡單類型    | 帶正負號的整數︰`sbyte`、`short`、`int`、`long` |
...
|                   | 64       | `ulong`   | 0...到 18446744073709551615 |
...
|                                  | `new {...}`       | 匿名物件初始設定式 |
|                                  | `new T[...]`      | 建立陣列 |
|                                  | `typeof(T)`       | 取得`System.Type`物件 `T` |
|                                  | `checked(x)`      | 在核取的內容中評估運算式 |
|                                  | `unchecked(x)`    | 在未核取的內容中評估運算式 |
|                                  | `default(T)`      | 取得型別的預設值 `T` |
|                                  | `delegate {...}`  | 匿名函式 (匿名方法) |
| 一元                            | `+x`              | 身分識別 |
|                                  | `-x`              | 否定 |
|                                  | `!x`              | 邏輯否定 |
...
|                                  | `x <= y`          | 小於或等於 |
|                                  | `x >= y`          | 大於或等於 |
|                                  | `x is T`          | 傳回`true`如果`x`是`T`，`false`否則 |
|                                  | `x as T`          | 傳回`x`型別為`T`，或`null`如果`x`不是 `T` |
| 相等                         | `x == y`          | 等於      |
|                                  | `x != y`          | 不相等 |
| 邏輯 AND                      | `x & y`           | 整數位元 AND、 布林邏輯 AND |
| 邏輯 XOR                      | `x ^ y`           | 整數位元 XOR、布林邏輯 XOR |
| 邏輯 OR                       | ' x | y'           | 整數位元 OR、布林邏輯 OR |
| 條件式 AND                  | `x && y`          | 會評估`y`只有當`x`是 `true` |
| 條件式 OR                   | ' x || y'          | 會評估`y`只有當`x`是 `false` |
| Null 聯合                  | `X ?? y`          | 評估為`y`如果`x`是`null`至`x`否則 |
| 條件式                      | `x ? y : z`       | 會評估`y`如果`x`是`true`，`z`如果`x`是 `false` |
| 指派或匿名函式 | `x = y`           | 指派 |
|                                  | `x op= y`         | 複合指派;支援的運算子 `*=` `/=` `%=` `+=` `-=` `<<=` `>>=` `&=` `^=` `|=` |
...
`List<T>
`類別會宣告兩個屬性：`Count`和`Capacity`，這分別是唯讀和讀寫。 以下是這些屬性的用法範例。
