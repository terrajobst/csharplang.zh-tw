...
這裡套用至擷取本機變數相同的技巧也可以用匿名函式轉換成運算式樹狀架構時： 運算式樹狀結構中儲存編譯器產生物件的參考，而且可以是本機變數的存取權以欄位存取這些物件表示。 此方法的優點是它可讓 「 提取 」 的本機變數，委派和運算式樹狀架構之間共用。
...
若要指派`d2`示範如何建立到 （抵銷變體） 的參數型別衍生程度較小且更多衍生 （共變數） 的傳回型別方法的委派。
