...
*  格式的非零值的有限集合`s * m * 2^e`，其中`s`為 1 或-1，和`m`和`e`取決於特定的浮點數型別： 如`float`，`0 < m < 2^24`和`-149 <= e <= 104`，以及`double`，`0 < m < 2^53`和`1075 <= e <= 970`。 反正規化浮點的數字會被視為有效的非零值。
