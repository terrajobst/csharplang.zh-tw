...
*  整數否定：
   ...
   結果計算減去`x`從零。 如果值`x`是最小的運算元類型可代表值 (-2 ^31 個`int`或-2 ^63 `long`)，然後數學否定`x`不是運算元的型別中可顯示。 如果這發生在`checked`內容中，`System.OverflowException`就會擲回; 內發生`unchecked`內容中，結果是運算元的值，且不會報告溢位。
